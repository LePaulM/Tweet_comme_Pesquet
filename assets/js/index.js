 ...
 *     
 * ajouter avec un bouton
 * grace auquel tu peux tweeter à nouveau la photo
 * 
 * mieux gérer le passage d'un côté à l'autre de la carte 
 * (par ex : si on passe d'une longitude positive à une longitude négative, on trace pas le trait)
 * 
 * */
...
function gui(latlng,oldLatlng) {
    ...
    var satellite_icon = L.icon({
        iconUrl: 'assets/css/images/satellite.png',
                ...
                $.ajax({
                    ...
                    complete: function (data) {
                        if (data.readyState === 4 && data.status === 200) {
                            ...
                            var satellite_icon = L.icon({
                                iconUrl: 'assets/css/images/camera.png',
